...
try {
    ...
    $query = $pdo->query("SELECT COUNT(*) AS NUMBERABS, CURRENT_DATE, substr(LASTNAMESTUDENT,1,15) AS LASTNAMESTUDENT,substr(FIRSTNAMESTUDENT,1,10) AS FIRSTNAMESTUDENT,N_STUDENT,TPSTUDENT,CODEUE FROM ABSENCE join STUDENT USING (N_STUDENT) JOIN MODULE USING (N_MODULE) join UE USING (N_UE) WHERE STATUSABSENCE=0 group by LASTNAMESTUDENT, FIRSTNAMESTUDENT, N_STUDENT, TPSTUDENT, CODEUE");
