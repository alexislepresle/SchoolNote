...
try {
    ...
    $query = $pdo->query("SELECT COUNT(*) AS NUMBERABS, CURRENT_DATE, LASTNAMESTUDENT,FIRSTNAMESTUDENT,N_STUDENT,TPSTUDENT,CODEUE FROM ABSENCE join STUDENT USING (N_STUDENT) JOIN MODULE USING (N_MODULE) join UE USING (N_UE) WHERE STATUSABSENCE=0 AND CODEUE=2.1 OR CODEUE=2.2 group by LASTNAMESTUDENT, FIRSTNAMESTUDENT, N_STUDENT, TPSTUDENT, CODEUE");
